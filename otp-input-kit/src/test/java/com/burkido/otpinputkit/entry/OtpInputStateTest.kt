...
/**
 * Unit tests for [OtpInputFieldState].
 *
...
class OtpInputStateTest {
    ...
    @Before
    fun setUp() {
        state = OtpInputFieldState(otpLength = 6)
    }
    ...
    fun `initial state with pre-filled value`() {
        val prefilledState = OtpInputFieldState(otpLength = 6, initialValue = "123")
    ...
    fun `initial state with full pre-filled value`() {
        val fullState = OtpInputFieldState(otpLength = 4, initialValue = "1234")
    ...
    fun `custom otpLength of 4 enforces limit`() {
        val shortState = OtpInputFieldState(otpLength = 4)
    ...
    fun `custom otpLength of 8 allows longer codes`() {
        val longState = OtpInputFieldState(otpLength = 8)
